...
A:学校校园网络支持有线和无线连接，不过有的宿舍区无线网络不稳定，建议有线连接，电脑没有有线接口的可以买一个USB转换器；如果选择使用电信宽带网络，电脑端需要下载安装认证软件，认证成功才可以上网，学校大多数地方覆盖了F-young无线网络。在少人抢和信号强的地方网速还是挺快的，手机端也需要下载一个app认证，可以同时一台电脑一台手机在线。这个电信的电脑认证软件暂时只有 windows 平台和 ios 平台的，并没有 linux 等平台的～(其实有，不过较为不稳定，所以不推广)。
